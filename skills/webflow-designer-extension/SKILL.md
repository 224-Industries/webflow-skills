...
license: MIT
compatibility: "@webflow/webflow-cli"
metadata:
...
1. **Initialize**: `webflow extension init <name> [template]` (templates: `react`, `typescript`, etc.)
2. **Install**: `cd <name> && npm install`
3. **Develop**: `npm run dev` (serves at localhost:1337)
4. **Test**: Install app on test site via Workspace Settings > Apps & Integrations > Develop
5. **Open**: Press "E" in Designer to open app panel, launch extension
6. **Bundle**: `npm run build` or `webflow extension bundle` for deployment
...
- **[references/cli-reference.md](references/cli-reference.md)**: CLI commands and options
...
- **`scripts/init-extension.sh`**: Quick project initialization with dependency installation
